...
def reset_session_keys(keys: List[str]) -> None:
    """Remove keys from st.session_state safely."""
    for k in keys:
        try:
...
def extract_text_from_pdf(uploaded_pdf, max_pages: int = 6) -> str:
    ...
    # Prefer PyMuPDF when available: tends to preserve text layout better (tables/captions).
    if _PYMUPDF_OK:
        try:
...
st.sidebar.markdown("---")
tab5_enabled = st.sidebar.checkbox(
    "Habilitar TAB 5 (PDF → Base)",
    value=True,
    help="Si tu deploy tiene restricciones o estás depurando, puedes desactivar temporalmente este módulo."
)
...
with tab5:
    ...
    if not tab5_enabled:
        st.info("TAB 5 está desactivado desde el panel izquierdo. Actívalo cuando quieras usar ingesta PDF → Base.")
    else:
    ...
    **Importante:** para guardar como **VALIDADO** se exige una **dosis > 0** y que confirmes explícitamente que la dosis viene del artículo.
        ...
        if not gs_enabled:
            ...
            if clear_btn:
                ...
                reset_session_keys([
                    "t5_doi_manual",
                    "t5_pdf_doi", "t5_pdf_fuente", "t5_pdf_verification_notes",
                    "t5_pdf_feedstock_v", "t5_pdf_ph_soil_v", "t5_pdf_mo_v", "t5_pdf_textura_v",
                    "t5_pdf_T_v", "t5_pdf_ph_bio_v", "t5_pdf_bet_v", "t5_pdf_dosis_v",
                    "t5_pdf_objetivo_v", "t5_pdf_tamano_v",
                    "t5_pdf_save_as_confirmed",
                    "t5_pdf_humedad_v", "t5_pdf_volatiles_v", "t5_pdf_cenizas_v", "t5_pdf_carbono_fijo_v",
                    "t5_pdf_o2_ppm_v", "t5_pdf_o2_temp_v", "t5_pdf_metodo_enfriamiento_v",
                    "t5_pdf_hc_ratio_v", "t5_pdf_oc_ratio_v",
                    "t5_pdf_tipo_articulo_v", "t5_pdf_year_v", "t5_pdf_title_v", "t5_pdf_journal_v",
                ])
                st.success("TAB 5 reiniciado (estado + extracción).")
